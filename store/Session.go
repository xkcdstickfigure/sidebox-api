...
func (s Store) SessionCreate(ctx context.Context, userId string, address string, userAgent string) (Session, error) {
	var session Session
	err := s.Conn.QueryRow(ctx, "insert into session (id, account_id, token, address, user_agent, created_at) "+
		"values ($1, $2, $3, $4, $5, now()) "+
		"returning id, account_id, token, address, user_agent, created_at",
		uuid.New(), userId, random.String(32), address, userAgent).
		Scan(&session.Id, &session.AccountId, &session.Token, &session.Address, &session.UserAgent, &session.CreatedAt)
	return session, err
}
